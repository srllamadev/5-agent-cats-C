...
if (typeof window !== 'undefined') {
  (function bootstrapFromEnv() {
    if (ENV.DEEPSEEK_API_KEY) ((k,v) => typeof window !== 'undefined' ? localStorage.setItem(k,v) : null)('apikey_deepseek', ENV.DEEPSEEK_API_KEY);
    if (ENV.CLAUDE_API_KEY)   ((k,v) => typeof window !== 'undefined' ? localStorage.setItem(k,v) : null)('apikey_claude',   ENV.CLAUDE_API_KEY);
    if (ENV.OPENAI_API_KEY)   ((k,v) => typeof window !== 'undefined' ? localStorage.setItem(k,v) : null)('apikey_openai',   ENV.OPENAI_API_KEY);
    if (ENV.PINATA_JWT)       ((k,v) => typeof window !== 'undefined' ? localStorage.setItem(k,v) : null)('apikey_pinata',   ENV.PINATA_JWT);
    if (ENV.SNOWTRACE_API_KEY)((k,v) => typeof window !== 'undefined' ? localStorage.setItem(k,v) : null)('apikey_snowtrace',ENV.SNOWTRACE_API_KEY);

    // Aplicar defaults de env.js sólo si no hay settings guardados aún
    const existing = ((k) => typeof window !== 'undefined' ? localStorage.getItem(k) : null)('audit_settings');
    if (!existing) {
      const envSettings = {
      ...
      ((k,v) => typeof window !== 'undefined' ? localStorage.setItem(k,v) : null)('audit_settings', JSON.stringify(envSettings));
    }
  })();
}
...
export function loadSettings() {
  try {
    const saved = ((k) => typeof window !== 'undefined' ? localStorage.getItem(k) : null)('audit_settings');
    return saved ? { ...DEFAULTS, ...JSON.parse(saved) } : { ...DEFAULTS };
  } catch { return { ...DEFAULTS }; }
}

export function saveSettings(settings) {
  ((k,v) => typeof window !== 'undefined' ? localStorage.setItem(k,v) : null)('audit_settings', JSON.stringify(settings));
}


// API key helpers (stored separately for security isolation)
export function getApiKey(provider) {
  return ((k) => typeof window !== 'undefined' ? localStorage.getItem(k) : null)(`apikey_${provider}`) || '';
}

export function setApiKey(provider, key) {
  if (key) {
    ((k,v) => typeof window !== 'undefined' ? localStorage.setItem(k,v) : null)(`apikey_${provider}`, key);
  } else {
    ((k) => typeof window !== 'undefined' ? localStorage.removeItem(k) : null)(`apikey_${provider}`);
  }
}
...
export function initLedger(auditId) {
  ...
  ((k,v) => typeof window !== 'undefined' ? sessionStorage.setItem(k,v) : null)('audit_ledger', JSON.stringify(ledger));
  return ledger;
}

export function getLedger() {
  try {
    return JSON.parse(((k) => typeof window !== 'undefined' ? sessionStorage.getItem(k) : null)('audit_ledger') || 'null');
  } catch { return null; }
}
...
export function saveAuditResult(result) {
  ((k,v) => typeof window !== 'undefined' ? localStorage.setItem(k,v) : null)('audit_result',    JSON.stringify(result));
}

export function saveDashboardPayload(payload) {
  ((k,v) => typeof window !== 'undefined' ? localStorage.setItem(k,v) : null)('dashboard_payload', JSON.stringify(payload));
}

export function getAuditResult() {
  try { return JSON.parse(((k) => typeof window !== 'undefined' ? localStorage.getItem(k) : null)('audit_result') || 'null'); } catch { return null; }
}

export function getDashboardPayload() {
  try { return JSON.parse(((k) => typeof window !== 'undefined' ? localStorage.getItem(k) : null)('dashboard_payload') || 'null'); } catch { return null; }
}
